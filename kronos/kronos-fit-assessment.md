...
> **Evidence note (this folder).** Assesses one question for the hackathon: *is
> Kronos an appropriate forecasting method for the Vestacy monthly-volume
> contract?* It does not change the data contract or the forecasting skill's
> method — it supplies evidence, a reproducible benchmark, and a recommendation.
> Companion notebook: [`vestacy_kronos_wmape_benchmark.py`](./vestacy_kronos_wmape_benchmark.py).
