...
## ☁️ Deploying on Azure (Microsoft Fabric Real-Time Intelligence)

A reference solution and architecture for running Kronos as a production,
streaming forecasting service on Azure — ingesting live market data, maintaining
rolling OHLCV candles, continuously forecasting with Kronos, and visualizing /
alerting in near-real-time — is documented in **[`docs/azure/`](./docs/azure/)**:

*   **[Architecture & design](./docs/azure/kronos-on-azure-fabric-rta.md)** —
    Fabric Eventstream/Eventhouse (RTI) as the data & candle-building backbone,
    Azure ML managed online endpoints (GPU) for `KronosPredictor` serving,
    Real-Time Dashboards + Data Activator for visualization/alerting, and Azure
    ML GPU jobs for the `finetune/` pipeline.
*   **[Reference implementation](./deploy/azure/)** — Eventhouse KQL (tables,
    OHLCV materialized views, forecast store), the Azure ML scoring endpoint, a
    Fabric orchestration notebook, and a Bicep infrastructure skeleton.
